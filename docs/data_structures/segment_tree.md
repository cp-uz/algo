...
$\text{build}$, $\text{update}$ va $\text{count_zero}$ funksiyalarini yig‘indi so‘rovi g‘oyalariga tayangan holda implementatsiya qilish ravshan.
...
Barg tugunlarni boshlang‘ich qiymat bilan to‘ldirish uchun bitta qiymat haqidagi ma’lumotni saqlovchi $\text{data}$ obyektini qaytaradigan yordamchi $\text{make_data}$ funksiyasini ham yaratamiz.
...
$\text{query}$ funksiyasi ham deyarli o‘zgarmaydi, faqat endi $\text{multiset}$ ning $\text{lower_bound}$ funksiyasini chaqirish kerak. $\text{std::lower_bound}$ random-access iteratorlar bilan ishlatilgandagina $O(\log n)$ vaqt ishlaydi.
...
$\text{find_kth}$ funksiyasi implementatsiyasida buning uchun ikkita tugun ko‘rsatkichini uzatib, joriy kesma sanog‘i yoki yig‘indisini shu tugunlardagi ikki sanoq yoki yig‘indi ayirmasi sifatida hisoblash mumkin.

Quyida o‘zgartirilgan $\text{build}$, $\text{update}$ va $\text{find_kth}$ funksiyalari berilgan.
